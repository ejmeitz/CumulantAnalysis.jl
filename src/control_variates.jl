...
#     Y1 = (X .- μX) .* Z
#     cov_XZ, cvd2 = cv_estimate_crossfit(Y1, cvs..., cvs_nz2...)
#     ∂X_∂T = cov_XZ / (kB * T^2)
...
#     # dXV = cov_XVV / (kB * T^2)
#     cvd3 = ControlVariateData(zeros(0), zeros(0), Float64(1.0))
#     dXV = cov(tmp, V2) / (kB * T^2)
...
#     # Estimate for ∂<X>/∂T
#     Y1 = (X .- μX) .* Z
#     cov_XZ = apply_cv(Y1, cvds[2], cvs..., cvs_nz2...) 
#     ∂X_∂T = cov_XZ / (kB * T^2)
